...
import { SubscribeMailRequest } from "../../types/report";
import defaultInstance from "../utils/instance";

const subscribeMailApi = async (body: SubscribeMailRequest): Promise<null> => {
  try {
    const res = await defaultInstance.post<ApiEnvelopeNullable<null>>(
      `/email-subscriptions`,
      body
    );
